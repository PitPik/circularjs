define('game-controller', ['game-service'], function(gameSrv) {  'use strict';
  ...
  function lookAround(model, item, checkAround, foundMarked) {
    gameSrv.lookAround(model, item.parentNode.index, item.index, function(row, col, foundItem) {
  ...
  function checkAll(items) {
    for (var x = items.length; x--; ) if (!items[x].isMine || items[x].mark !== 'marked') return;
    return true;
  }

  function checkItem(model, item, mark) {
    if (item.isProcessed && mark !== undefined) {
      delete checkItem._win;
      if (lookAround(model, item, false, 0) === item.surrounding) lookAround(model, item, true);
